...
#
# Environment:
#   ICONS_FLUENTUI_SYSTEM_ICONS_SRCDIR       project root (default: packaging/../..)
#   ICONS_FLUENTUI_SYSTEM_ICONS_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
ICONS_FLUENTUI_SYSTEM_ICONS_SRCDIR=${ICONS_FLUENTUI_SYSTEM_ICONS_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$ICONS_FLUENTUI_SYSTEM_ICONS_SRCDIR/.config/icons-fluentui-system-icons" "$HOME/.config/icons-fluentui-system-icons"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${ICONS_FLUENTUI_SYSTEM_ICONS_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${ICONS_FLUENTUI_SYSTEM_ICONS_PACKAGING:-}")"
...
cmd_run() {
    ...
    if [[ ${ICONS_FLUENTUI_SYSTEM_ICONS_FORCE_LOCAL:-} == 1 ]]; then
        exec "$@"
    fi
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $ICONS_FLUENTUI_SYSTEM_ICONS_SRCDIR/.config/icons-fluentui-system-icons/$k.build-host" >&2
        echo "    $HOME/.config/icons-fluentui-system-icons/$k.build-host" >&2
    done
    return 2
}
